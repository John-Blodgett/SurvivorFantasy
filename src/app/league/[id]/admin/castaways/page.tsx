          ...
          {active.length === 0 ? (
            ...
            <ul className="space-y-2">
              {active.map((c: Castaway) => (
                <CastawayRow key={c.id} castaway={c} action="eliminate" leagueId={leagueId} />
              ))}
            </ul>
          )}
        </section>
          ...
          <section aria-labelledby="eliminated-heading">
            ...
            <ul className="space-y-2">
              {eliminated.map((c: Castaway) => (
                <CastawayRow key={c.id} castaway={c} action="restore" leagueId={leagueId} />
              ))}
            </ul>
          </section>
        )}
...
function CastawayRow({ castaway, action, leagueId }: { castaway: Castaway; action: "eliminate" | "restore"; leagueId: string }) {
  return (
